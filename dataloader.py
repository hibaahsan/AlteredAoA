...
class DataLoader(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        self.use_text = getattr(opt, 'use_text', 0)
    ...
    def __getitem__(self, index):
        ...
        if self.use_att:
            ...
            if self.use_text:
                no_text_ids = np.load(self.opt.no_text_ids)
                if self.info['images'][ix]['id'] in no_text_ids:
            ...
            if self.use_text and self.norm_att_feat:
                text_feat = text_feat / np.linalg.norm(text_feat, 2, 1, keepdims=True)
            if self.use_box:
